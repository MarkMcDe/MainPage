...
if (count($decoded_response["items"])>0){
...
//Jan 9: Adding id tags to elements for better CSS formatting
    ...
    echo "<span class='timeTitle'><strong>" . $Gdate->format('g:i') . " ";
	$Ampm2 = ($Gend->format('a'));
...
//Inserts beer mug emoji
    ...
    echo " <span class='timeTitle'><strong>" . $decoded_response["items"][$i]["summary"] . "</strong></strong></span><br/>";

echo "<span class='place'>" . $decoded_response["items"][$i]["location"] . " ";
...
echo "<span class='theStuff'>" . nl2br($decoded_response["items"][$i]["description"]) . "</a></strong></em></span></p><p>";

} 
} else {
	echo ("<strong>No events for today.</strong><br/>But please check back, we update continuously.");
}
//    echo "<br/><hr></div>";
//*/
    ?>
